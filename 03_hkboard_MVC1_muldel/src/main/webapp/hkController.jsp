...
<body>
   ...
   if (command.equals("boardList")) { //글목록 요청 처리
      ...
      if(isS){
	    ...
	    <!-- <script type="text/javascript"> 
	       alert("글을 추가합니다.");
	      location.href = "hkController.jsp?command=boardList"; //글을 추가하고, 글 목록 페이지로 이동하기 
	    </script>  -->
      ...
      <%
	      ...
	      <script type="text/javascript">
	         alert("글 추가 실패!");
	         location.href = "hkController.jsp?command=insertBoardForm"; //글 추가 실패하면, 글 추가 페이지로 이동하기
	      </script>
      	 <%
         }
  ...
  //  	  } else{
        	  ...
        	  if(isS){
        		  ...
        		  response.sendRedirect("error.jsp?msg=" + URLEncoder.encode("여러글삭제실패","utf-8"));
        	  }
    	//  }
