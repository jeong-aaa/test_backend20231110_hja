...
//		Statement -> ?파라미터를 제공X
		...
		try {
			...
			while(rs.next()) {//rs객체안에 데이터가있는지 여부를 확인 
				BoardDto ddto = new BoardDto();//행단위로 저장
				ddto.setSeq(rs.getInt(1));//DB에서 인덱스 체계는 1부터 시작
				ddto.setId(rs.getString(2));//
				ddto.setTitle(rs.getString(3));
				ddto.setContent(rs.getString(4));
				ddto.setRegDate(rs.getDate(5));
				list.add(ddto);
	...
	//반환 타입 X -> 테이블을 수정 
	public boolean updateBoard(BoardDto ddto) {
		int count = 0 ;
		Connection conn = null;
		PreparedStatement psmt = null;
		...
		try {
			conn = getConnection();
			psmt = conn.prepareStatement(sql);
			psmt.setString(1, ddto.getTitle());
			psmt.setString(2, ddto.getContent());
			psmt.setInt(3, ddto.getSeq());
