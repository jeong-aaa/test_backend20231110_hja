 ...
 */
	function setCookie(name,value,expires,domain,path,secure){
      ...
      var cookies='';
      // 인코딩처리 3가지
      // - escape(): ASCII코드로 변환, 나머지는 유니코드로 변환 -> 이제 안 씀
      // - encodeURI(): 주소와 관련된 특수문자 https://hankyung.com?key=value&key/value
      //              들을 제외하고 인코딩 처리 -> URI전체를 처리할 때 사용
      // - encodeURIComponent(): 모든 문자를 인코딩 처리 -> 파라미터를 처리할 때 사용
      ...
      if(domain){
        cookies+=";domain="+domain; //특정도메인, 하위도메인에서만 사용: hk.com, sub.hk.com
     }
     if(path){
