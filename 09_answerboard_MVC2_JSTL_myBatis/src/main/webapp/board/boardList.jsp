               ...
               <c:forEach items="${lists}" var="dto">
                  <tr>
                     <td>
                           ...
                           <td>${dto.id}</td>
                           <td>
                              <c:set var="title" value="${fn:length(dto.title)>10?fn:substring(dto.title,0,10)+='...':dto.title}"/>
                              <c:choose>
                                 <c:when test="${dto.delflag=='Y'}">--삭제된 글입니다.--</c:when>
                  <c:otherwise>
...
<%--                </c:forEach> --%>
         ...
         <tr>
            <td colspan="10">
               <button type="button" onclick="location.href='insertForm.board'">추가</button>
